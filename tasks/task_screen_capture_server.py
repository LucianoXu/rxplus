...
def build_parser(subparsers: argparse._SubParsersAction):
    parser = subparsers.add_parser(
        "screen_capture_server", help="try the screen_capture server."
    )
    parser.add_argument(
        "--fps",
        type=float,
        default=10.0,
        help="The frames per second for screen capture.",
    )
...
def task(parsed_args: argparse.Namespace):
    ...
    source = create_screen_capture(
        fps=parsed_args.fps, scheduler=ThreadPoolScheduler(1)
    )

    fps_monitor = FPSMonitor(interval=1.0)
    bandwidth_monitor = BandwidthMonitor(
        interval=1.0, scale=1 / (1024.0 * 1024.0), unit="MB/s"
    )
