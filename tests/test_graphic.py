...
class TestPNGCodec:
    ...
    def test_png_compression_affects_size(self):
        ...
        # Use a gradient image that compresses well
        # (random data doesn't compress predictably)
...
class TestScreenCapture:
    ...
    def test_fps_timing(self, mock_mss):
        ...
        if len(received) >= 2:
            ...
            intervals = [
                received[i + 1] - received[i] for i in range(len(received) - 1)
            ]
    ...
    def test_uses_thread_scheduler_outside_loop(self, mock_mss):
        ...
        subscription = obs.subscribe(
            on_next=lambda x: thread_ids.append(threading.get_ident())
        )
