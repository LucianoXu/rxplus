...
class TestRxMicrophone:
    ...
    def test_emits_on_callback(self, mock_pyaudio, setup_event_loop):
        ...
        callback = call_kwargs.kwargs.get("stream_callback") or call_kwargs[1].get(
            "stream_callback"
        )
    ...
    def test_format_configuration(self, mock_pyaudio, setup_event_loop):
        ...
        RxMicrophone(format="Int16", sample_rate=44100, channels=2)
    ...
    def test_device_index_passed_to_pyaudio(self, mock_pyaudio, setup_event_loop):
        ...
        RxMicrophone(format="Float32", sample_rate=48000, channels=1, device_index=3)
    ...
    def test_device_index_none_by_default(self, mock_pyaudio, setup_event_loop):
        ...
        RxMicrophone(format="Float32", sample_rate=48000, channels=1)
...
class TestRxSpeaker:
    ...
    def test_format_configuration(self, mock_pyaudio):
        ...
        RxSpeaker(format="Int16", sample_rate=44100, channels=2)
    ...
    def test_device_index_passed_to_pyaudio(self, mock_pyaudio):
        ...
        RxSpeaker(format="Float32", sample_rate=48000, channels=1, device_index=2)
    ...
    def test_device_index_none_by_default(self, mock_pyaudio):
        ...
        RxSpeaker(format="Float32", sample_rate=48000, channels=1)
...
class TestSaveWavFile:
    ...
    def test_closes_on_completed(self, tmp_path):
        ...
        observer = SaveWavFile(
            str(wav_path), format="Float32", sample_rate=48000, channels=1
        )
...
def test_wavfile_roundtrip(tmp_path):
    ...
    source_data = (
        np.sin(np.linspace(0, 2 * np.pi, 4800)).astype(np.float32).reshape(-1, 1)
    )
    ...
    dest_observer = save_wavfile(
        str(dest_path), format="Float32", sample_rate=48000, channels=1
    )
...
class TestConvertChannels:
    ...
    def test_downmix_multichannel(self):
        ...
        # 4-channel audio
        quad = np.array([[1.0, 2.0, 3.0, 4.0], [0.0, 0.0, 0.0, 0.0]], dtype=np.float32)
