...
class StreamTable:
        ...
        with self._lock:
            if stream_id in self._streams:
                ...
                if info.state != StreamState.CLOSED:
                    raise ValueError(
                        f"Stream {stream_id.hex()} already open"
                        f" (state={info.state.name})"
                    )
    ...
    def get_stream(self, stream_id: bytes) -> StreamInfo | None:
    ...
    def list_streams(self, state: StreamState | None = None) -> list[bytes]:
        ...
        with self._lock:
            ...
            return [sid for sid, info in self._streams.items() if info.state == state]
    ...
    def active_count(self) -> int:
        ...
        with self._lock:
            return sum(
                1 for info in self._streams.values() if info.state == StreamState.ACTIVE
            )

    def remove_stream(self, stream_id: bytes) -> StreamInfo | None:
