...
from ..ws import RxWSServer, RxWSClient, RetryPolicy, ConnectionState as WSConnectionState
from ..utils import TaggedData
from ..duplex import make_duplex
from ..telemetry import get_default_providers, OTelLogger

from .connection import Connection, ConnectionState, HelloPayload
...
class GatewayNode(Generic[F]):
    ...
    def __init__(
        ...
        Args:
            ...
            retry_policy: Configurable retry behavior with exponential backoff.
                If None, uses default RetryPolicy with base_delay=2.0 and infinite retries.
        ...
        self._otel_logger.info(f"GatewayNode started: {self.node_id.hex()} listening on {host}:{port}")
    ...
    def _setup_server_pipeline(self):
        ...
        self._disposables.add(
            self._server.pipe(
                ...
                on_error=lambda e: self._otel_logger.error(f"Server pipeline error: {e}"),
            )
        )
    
    def _handle_inbound_data(self, tagged: TaggedData) -> Optional[TaggedFrame[F]]:
        ...
        with self._connections_lock:
            if path not in self._inbound_path_to_conn:
                ...
                self._otel_logger.info(f"Accepted inbound connection: {conn_id.hex()[:16]} from {path}")
        ...
        if conn is None:
            self._otel_logger.warning(f"Received data for unknown connection path: {path}")
            return None
        ...
        if frame.is_hello():
            return self._handle_hello(conn, frame)
        elif frame.is_hello_ack():
            return self._handle_hello_ack(conn, frame)
        
        # Only forward non-handshake events if connection is READY
        if conn.state != ConnectionState.READY:
            self._otel_logger.warning(f"Received data on non-ready connection: {conn_id.hex()[:16]}")
            return None
    ...
    def _send_hello_ack_to_path(self, path: str):
        ...
        self._server.on_next(TaggedData(path, self._framing_cls.encode(hello_ack_frame)))
        self._otel_logger.debug(f"Sent HELLO_ACK to path {path}")
    
    def _handle_hello(self, conn: Connection, frame: F) -> Optional[TaggedFrame[F]]:
        """Handle incoming HELLO message."""
        peer_node_id = frame.get_hello_node_id()
        if peer_node_id is None or len(peer_node_id) != 16:
            self._otel_logger.error(f"Invalid HELLO payload")
            return None
        
        conn.peer_node_id = peer_node_id
        conn._hello_received = True
        
        self._otel_logger.debug(f"Received HELLO from {peer_node_id.hex()[:16]} on conn {conn.conn_id.hex()[:16]}")
    ...
    def _handle_hello_ack(self, conn: Connection, frame: F) -> Optional[TaggedFrame[F]]:
        """Handle incoming HELLO_ACK message."""
        peer_node_id = frame.get_hello_node_id()
        if peer_node_id is None or len(peer_node_id) != 16:
            self._otel_logger.error(f"Invalid HELLO_ACK payload")
            return None
        ...
        self._otel_logger.debug(f"Received HELLO_ACK from {peer_node_id.hex()[:16]} on conn {conn.conn_id.hex()[:16]}")
    ...
    def _check_handshake_complete(self, conn: Connection):
        ...
        should_emit = False
        with self._connections_lock:
            if conn._hello_received and conn._hello_ack_received and conn.state == ConnectionState.HANDSHAKING:
                conn.state = ConnectionState.READY
                should_emit = True
                self._otel_logger.info(
                    f"Handshake complete: {conn.conn_id.hex()[:16]} ↔ {conn.peer_node_id.hex()[:16] if conn.peer_node_id else 'unknown'}"
                )
    ...
    def connect(self, host: str, port: int) -> bytes:
        ...
        # Use a unique path based on conn_id to distinguish this connection on the server
        ...
        self._otel_logger.info(f"Connecting to {host}:{port} (conn_id={conn_id.hex()[:16]})")
    ...
    def _setup_client_pipeline(self, conn: Connection):
        ...
        conn._disposables.add(
            ws_client.pipe(
                ops.map(lambda data: self._handle_outbound_data(conn, data)),
                ops.filter(lambda x: x is not None),
            ).subscribe(
                on_next=lambda tagged: self._sink.on_next(tagged),
                on_error=lambda e: self._otel_logger.error(f"Client pipeline error for {conn.conn_id.hex()[:16]}: {e}"),
            )
        )
        ...
        conn._disposables.add(
            ws_client.connection_state.subscribe(
                ...
                on_error=lambda e: self._otel_logger.error(f"Connection state error for {conn.conn_id.hex()[:16]}: {e}"),
            )
        )
    
    def _handle_ws_connection_state(self, conn: Connection, ws_state: WSConnectionState):
        ...
        self._otel_logger.debug(f"WS state change for {conn_id_short}: {ws_state.value}")
        ...
        if ws_state == WSConnectionState.CONNECTED:
            ...
            self._otel_logger.info(f"WS connected, starting handshake for {conn_id_short}")
            ...
            if conn.state == ConnectionState.READY:
                self._otel_logger.info(f"WS disconnected, connection {conn_id_short} no longer READY")
            ...
            # WebSocket closed permanently (max retries exhausted or on_completed called)
            if conn.state != ConnectionState.CLOSED:
                self._otel_logger.info(f"WS closed permanently for {conn_id_short}")
                conn.state = ConnectionState.CLOSED
                self._on_disconnection.on_next(conn.conn_id)
    
    def _handle_outbound_data(self, conn: Connection, data: bytes) -> Optional[TaggedFrame[F]]:
        ...
        # Only forward non-handshake events if connection is READY
        if conn.state != ConnectionState.READY:
            self._otel_logger.warning(f"Received data on non-ready outbound connection: {conn.conn_id.hex()[:16]}")
            return None
    ...
    def _find_path_for_conn(self, conn_id: bytes) -> Optional[str]:
    ...
    def disconnect(self, conn_id: bytes) -> bool:
        ...
        if conn is None:
            self._otel_logger.warning(f"Disconnect: connection not found: {conn_id.hex()[:16]}")
            return False
    ...
    def send_to(self, conn_id: bytes, frame: F) -> bool:
        ...
        if conn is None:
            self._otel_logger.warning(f"send_to: connection not found: {conn_id.hex()[:16]}")
            return False
        
        if conn.state != ConnectionState.READY:
            self._otel_logger.warning(f"send_to: connection not ready: {conn_id.hex()[:16]} (state={conn.state.name})")
            return False
